...
begin
  ...
  -- Treat stored times as UTC if they lack timezone info to avoid TZ mismatch
  select min(((r.value->>'time')::timestamp AT TIME ZONE 'UTC'))
    into earliest_race
  ...
  -- Only allow update if now is at least 10 minutes before the earliest race
  return now_time < (earliest_race - interval '10 minutes');
end;
